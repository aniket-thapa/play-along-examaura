...
router.get('/', async (req, res) => {
  try {
    ...
    const attempts = await Attempt.find({
      ...
      quiz: { $in: quizIds },
      isSubmitted: true,
    }).select('quiz score totalMarks isSubmitted autoSubmitted').lean();
    ...
    const result = quizzes.map((quiz) => ({
      ...
      attempt: attemptMap[quiz._id.toString()] || null,
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (quiz.isLocked) {
      return res.status(403).json({ success: false, message: 'This quiz is currently locked' });
    }
...
router.post('/:id/start', async (req, res) => {
  try {
    ...
    if (quiz.isLocked) return res.status(403).json({ success: false, message: 'Quiz is locked' });

    // Check if already submitted
    const submitted = await Attempt.findOne({ user: req.user._id, quiz: quiz._id, isSubmitted: true });
    if (submitted) {
      return res.status(409).json({ success: false, message: 'You have already submitted this quiz' });
    }

    // Get or create in-progress attempt
    let attempt = await Attempt.findOne({ user: req.user._id, quiz: quiz._id, isSubmitted: false });
    if (!attempt) {
      attempt = await Attempt.create({
        user: req.user._id,
        quiz: quiz._id,
        totalMarks: quiz.questions.reduce((s, q) => s + (q.marks || 1), 0),
        answers: quiz.questions.map((q) => ({ questionId: q._id, selectedOptionIndex: null })),
      });
    }
...
router.post('/:id/submit', validate(schemas.submitQuiz), async (req, res) => {
  try {
    const { answers, autoSubmitted, autoSubmitReason, tabSwitchCount, timeTakenSeconds } = req.body;
    const quiz = await Quiz.findById(req.params.id);
    if (!quiz) return res.status(404).json({ success: false, message: 'Quiz not found' });

    // Allow submission even if quiz was relocked (edge case)
    const attempt = await Attempt.findOne({ user: req.user._id, quiz: quiz._id, isSubmitted: false });
    if (!attempt) {
      const submitted = await Attempt.findOne({ user: req.user._id, quiz: quiz._id, isSubmitted: true });
      if (submitted) {
    ...
    attempt.autoSubmitReason = autoSubmitReason || (autoSubmitted ? 'time_expired' : 'manual');
