...
import { Button, Logo, ProgressBar, Spinner, ScoreRing } from '../../components/ui';
...
const InstructionScreen = ({ quiz, onStart, onBack }) => (
  <div className="min-h-screen bg-night-900 relative flex flex-col">
    ...
    <div className="relative z-10 flex flex-col flex-1 px-5 py-8 max-w-lg mx-auto w-full">
      <button onClick={onBack} className="flex items-center gap-2 text-[#8890b8] hover:text-white text-sm mb-8 -ml-1">
        <span>←</span> Dashboard
      </button>

      <div className="flex-1">
        <div className="text-center mb-8">
          <div className="w-20 h-20 rounded-2xl bg-brand-300/10 border border-brand-300/20 flex items-center justify-center text-4xl mx-auto mb-4">📋</div>
          <h1 className="font-display font-bold text-2xl text-gradient mb-1">{quiz.title}</h1>
          {quiz.description && <p className="text-[#8890b8] text-sm">{quiz.description}</p>}
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            { icon: '⏱', label: 'Duration', value: `${Math.floor(quiz.durationSeconds / 60)}m` },
            { icon: '🏆', label: 'Points', value: quiz.totalMarks || quiz.questions.reduce((s, q) => s + (q.marks || 1), 0) },
          ].map((s) => (
            <div key={s.label} className="glass rounded-xl p-3 text-center">
              <div className="text-2xl mb-1">{s.icon}</div>
              <div className="font-display font-bold text-lg text-[#e8eaf6]">{s.value}</div>
        ...
        <div className="glass rounded-2xl p-5 mb-6">
          <h2 className="font-display font-bold text-sm text-brand-300 uppercase tracking-widest mb-4">Instructions</h2>
          <div className="space-y-3">
            {[
              { icon: '⏰', text: `Total time: ${Math.floor(quiz.durationSeconds / 60)} minutes. Quiz auto-submits when time runs out.` },
              { icon: '✅', text: 'Select one option per question. You can change or deselect your answer before submitting.' },
              { icon: '🚫', text: `Do not switch tabs or leave the browser. After ${MAX_TAB_SWITCHES} violations, your quiz is auto-submitted.` },
              { icon: '📤', text: 'Click "Submit Quiz" when done or wait for the timer to expire.' },
              { icon: '⚠️', text: 'Once submitted, you cannot reattempt the quiz.' },
            ].concat(
              quiz.instructions
              ...
              <div key={i} className="flex items-start gap-3">
                ...
                <p className="text-[#8890b8] text-sm leading-relaxed">{item.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="safe-bottom">
        ...
        <p className="text-center text-[#8890b8] text-xs mt-3">Timer starts immediately after clicking</p>
      </div>
    </div>
  </div>
);
...
const ResultScreen = ({ result, quizTitle, onBack }) => {
  ...
  const msg = pct >= 80 ? ['🎉 Excellent!', 'Outstanding performance!'] :
               pct >= 60 ? ['👍 Good Job!', 'Solid attempt!'] :
               pct >= 40 ? ['📚 Keep Going!', 'Room to improve!'] :
  ...
  return (
    <div className="min-h-screen bg-night-900 relative flex flex-col">
      ...
      <div className="relative z-10 flex flex-col flex-1 px-5 py-8 max-w-lg mx-auto w-full items-center justify-center">
        ...
        >
          <h2 className="font-display font-bold text-xl text-gradient mb-2">{quizTitle}</h2>
          <p className="text-[#8890b8] text-sm mb-8">Quiz Completed</p>

          <div className="flex justify-center mb-8">
            <ScoreRing score={result.score} total={result.totalMarks} size={140} />
          </div>

          <h1 className="font-display font-bold text-3xl text-[#e8eaf6] mb-1">{msg[0]}</h1>
          ...
          <div className="glass rounded-2xl p-5 mb-8 space-y-4">
            {[
              ...
              { label: 'Time Taken', value: result.timeTakenSeconds ? `${Math.floor(result.timeTakenSeconds / 60)}m ${result.timeTakenSeconds % 60}s` : 'N/A' },
              ...(result.autoSubmitted ? [{ label: 'Submitted By', value: result.autoSubmitReason === 'time_expired' ? '⏰ Timer Expired' : '🚫 Tab Switch', warn: true }] : []),
            ].map((row) => (
              <div key={row.label} className="flex justify-between items-center border-b border-white/5 pb-3 last:border-0 last:pb-0">
                ...
                <span className={`font-semibold text-sm ${row.warn ? 'text-amber-400' : 'text-[#e8eaf6]'}`}>{row.value}</span>
              </div>
            ))}
          </div>

          <Button onClick={onBack} fullWidth variant="secondary" size="lg">← Back to Dashboard</Button>
        </motion.div>
      </div>
    </div>
  );
...
export default function QuizPage() {
  ...
  const submitQuiz = useCallback(async (autoSubmit = false, reason = 'manual') => {
  ...
  const startQuiz = async () => {
    try {
      ...
      // Init answers
      const initAns = {};
      data.data.questions.forEach((q) => { initAns[q._id] = null; });
      setAnswers(initAns);
    ...
    const handleVisibility = () => {
      if (document.hidden) {
        ...
        if (tabSwitchRef.current >= MAX_TAB_SWITCHES) {
          toast.error(`Auto-submitting: too many tab switches (${MAX_TAB_SWITCHES})`);
          submitQuiz(true, 'tab_switch');
        } else {
          const remaining = MAX_TAB_SWITCHES - tabSwitchRef.current;
          toast.error(`⚠️ Tab switch detected! ${remaining} warning${remaining > 1 ? 's' : ''} left.`);
        }
      }
    };
  ...
  const timerClass = timeLeft <= 30 ? 'timer-danger' : timeLeft <= 60 ? 'timer-warning' : 'timer-safe';
  ...
  if (phase === 'instruction') return <InstructionScreen quiz={quiz} onStart={startQuiz} onBack={() => navigate('/dashboard')} />;

  if (phase === 'result') return <ResultScreen result={result} quizTitle={quiz?.title} onBack={() => navigate('/dashboard')} />;
        ...
        <div className="max-w-lg mx-auto px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-3">
              {tabSwitches > 0 && (
                <span className="text-amber-400 text-xs font-semibold">⚠️ {tabSwitches}/{MAX_TAB_SWITCHES}</span>
              )}
          ...
          <ProgressBar value={quiz.durationSeconds - timeLeft} max={quiz.durationSeconds} />
        </div>
      </div>
      ...
      <div className="relative z-10 flex-1 max-w-lg mx-auto w-full px-4 py-5 flex flex-col">
        ...
        <div className="flex items-center justify-between mb-4">
          <span className="text-[#8890b8] text-xs">
            Question <span className="text-[#e8eaf6] font-bold">{currentQ + 1}</span> of {quiz.questions.length}
          </span>
          <span className="text-[#8890b8] text-xs">
            Answered: <span className="text-brand-300 font-bold">{answeredCount}</span>/{quiz.questions.length}
          </span>
        </div>

        <ProgressBar value={currentQ + 1} max={quiz.questions.length} className="mb-5" />
          ...
          >
            <div className="glass rounded-2xl p-5 mb-5">
              <div className="flex items-start gap-3 mb-1">
                ...
                <p className="font-display font-semibold text-base text-[#e8eaf6] leading-snug pt-0.5">{question.text}</p>
              </div>
              {question.marks > 1 && (
                <p className="text-xs text-[#8890b8] mt-2 ml-10">{question.marks} points</p>
              )}
            </div>
            ...
            <div className="space-y-3">
              {question.options.map((opt, i) => {
                ...
                return (
                  ...
                  >
                    <span className={`shrink-0 w-7 h-7 rounded-full border-2 flex items-center justify-center text-sm font-bold transition-all
                      ${selected ? 'bg-brand-300 border-brand-300 text-night-900' : 'border-white/20 text-[#8890b8]'}`}>
                      {selected ? '✓' : String.fromCharCode(65 + i)}
                    </span>
                    <span className={`text-sm font-body ${selected ? 'text-brand-300 font-medium' : 'text-[#ccd6f6]'}`}>
                      {opt.text}
                    </span>
                  </motion.button>
                );
                ...
                if (unanswered > 0) {
                  toast((t) => (
                    <div className="text-sm">
                      <p className="font-semibold mb-2">{unanswered} question{unanswered > 1 ? 's' : ''} unanswered!</p>
                      <div className="flex gap-2">
                        <button
                          onClick={() => { toast.dismiss(t.id); submitQuiz(false, 'manual'); }}
                          className="px-3 py-1 bg-brand-300 text-night-900 rounded-lg text-xs font-bold"
                        >Submit Anyway</button>
                        <button onClick={() => toast.dismiss(t.id)} className="px-3 py-1 bg-white/10 rounded-lg text-xs">Review</button>
                      </div>
                    </div>
