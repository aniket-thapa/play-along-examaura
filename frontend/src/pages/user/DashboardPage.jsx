...
import { Badge, Logo, ScoreRing, Spinner, EmptyState } from '../../components/ui';
...
const QuizCard = ({ quiz, index, onClick }) => {
  const { attempt, isLocked, title, description, questionCount, totalMarks, durationSeconds, coverColor } = quiz;
  ...
  return (
    ...
    >
      {/* Color accent bar */}
      <div className="h-1 w-full" style={{ background: isLocked ? '#1a2060' : coverColor }} />

      <div className="p-5">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0">
            <h3 className="font-display font-bold text-base text-[#e8eaf6] truncate">{title}</h3>
            {description && <p className="text-[#8890b8] text-xs mt-0.5 line-clamp-2">{description}</p>}
          </div>
          {isAttempted ? (
            <ScoreRing score={attempt.score} total={attempt.totalMarks} size={56} />
          ) : isLocked ? (
            <div className="shrink-0 w-10 h-10 rounded-xl bg-night-700 flex items-center justify-center text-xl lock-pulse">🔒</div>
          ) : (
            <div className="shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: `${coverColor}20` }}>
              ⚡
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="text-[#8890b8] text-xs flex items-center gap-1">
            <span className="text-brand-300/70">⏱</span> {formatDuration(durationSeconds)}
          </span>
          <span className="text-[#8890b8] text-xs flex items-center gap-1">
...
export default function DashboardPage() {
  ...
  const totalScore = quizzes.reduce((s, q) => s + (q.attempt?.score || 0), 0);
  const totalMarks = quizzes.reduce((s, q) => s + (q.attempt ? q.attempt.totalMarks : 0), 0);
      ...
      <div className="relative z-10">
        ...
        <div className="sticky top-0 z-20 glass-dark border-b border-white/5 px-5 py-3">
          <div className="flex items-center justify-between max-w-lg mx-auto">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => fetchQuizzes(true)} className="text-[#8890b8] hover:text-brand-300 transition-colors text-lg" title="Refresh">
                ↻
              </button>
              <button
        ...
        <div className="max-w-lg mx-auto px-5 py-6 pb-10">
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
            ...
            <h1 className="font-display font-bold text-2xl text-gradient">{user?.name} 👋</h1>
          </motion.div>
          ...
          >
            {[
              ...
              { label: 'Score', value: `${totalScore}/${totalMarks}`, icon: '🏆' },
              { label: 'Available', value: unlocked, icon: '🔓' },
            ].map((stat) => (
              <div key={stat.label} className="glass rounded-xl p-3 text-center">
                <div className="text-xl mb-1">{stat.icon}</div>
                <div className="font-display font-bold text-base text-[#e8eaf6]">{stat.value}</div>
                <div className="text-[#8890b8] text-[10px] font-medium mt-0.5">{stat.label}</div>
              </div>
            ))}
          </motion.div>
          ...
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display font-bold text-base text-[#e8eaf6]">
              Quizzes <span className="text-[#8890b8] font-body font-normal text-sm">({quizzes.length})</span>
            </h2>
            <span className="text-xs text-[#8890b8]">Auto-refreshes every 30s</span>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {quizzes.map((quiz, i) => (
                <QuizCard key={quiz._id} quiz={quiz} index={i} onClick={handleQuizClick} />
              ))}
            </div>
          )}
